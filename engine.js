...
function gaussianConcentration(x, y, z, stackParameters, envParameters) {
  ...
  return Math.floor(
    (1000000 *
      stackParameters["discharge"] *
      Math.exp((y * y) / (2 * sigY * sigY)) *
      Math.exp(
        -Math.pow(z - stackParameters["height"], 2) / (2 * sigZ * sigZ)
      )) /
      (Math.PI * 2 * sigY * sigZ * envParameters["windVelocity"])
  );
}
...
function interpolationHelper(xList, yList, x) {
  ...
  for (let i = 0; i < xList.length; i++) {
    if (xList[i] > x) {
      return (
        yList[i] + ((yList[i + 1] - yList[i]) * x) / (xList[i + 1] - xList[i])
      );
    }
  }
}

export { gaussianConcentration };
